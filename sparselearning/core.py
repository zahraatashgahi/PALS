...
class Masking(object):
    def __init__(self, optimizer, prune_rate=0.3, growth_death_ratio=1.0,
    prune_rate_decay=None, death_mode='magnitude', growth_mode='momentum', 
    redistribution_mode='momentum', threshold=0.001, args=None, train_loader=None, dense_params = 0, device=None):
        ...
        self.sparsity_level = 1 - args.init_density
    ...
    def init(self, mode='ER', density=0.05, erk_power_scale=1.0, grad_dict=None):
        if self.args.method == 'GMP':
            ...
            for name, weight in self.masks.items():
                ...
                if 'classifier' in name or "decoder.projection.weight" in name:
            ...
            print(f"Overall sparsity {total_nonzero / self.dense_params}")
        ...
        print('Total parameters under sparsity level of {0}: {1}'.format(density, sparse_size / self.dense_params))
        ...
        if self.prune_every_k_steps is not None:
                if self.args.method == 'GraNet':
                    if self.steps >= (self.args.init_prune_epoch * len(self.loader)*self.args.multiplier) and self.steps % self.prune_every_k_steps == 0:
                        ...
                        for name, weight in self.masks.items():
                            sparse_size += (weight != 0).sum().int().item()
                        print('\n\nTotal parameters under sparsity level of {0}: {1}'.format(1 - sparse_size/ self.dense_params, sparse_size))
                        print('Total parameters of dense model: {0}'.format(self.dense_params))
                        self.sparsity_level = 1 - sparse_size/ self.dense_params
                        #self.sparsity_level = 1 - sparse_size/ total_size
                    ...
                    if self.steps >= (self.args.init_prune_epoch * len(self.loader)*self.args.multiplier) \
                            and self.steps % self.prune_every_k_steps == 0\
                            and curr_prune_iter < final_iter:
                        print('******************************************************')
                        ...
                        if status == "prune":
                            self.truncate_weights_prune()
                        elif status == "grow":
                            self.truncate_weights_grow()
                        elif status == "stable":
                            self.truncate_weights()
                        self.print_nonzero_counts()
                        total_size = 0
                        ...
                        for name, weight in self.masks.items():
                            sparse_size += (weight != 0).sum().int().item()
                        print('Total parameters under sparsity level of {0}: {1}'.format(1 - sparse_size/ self.dense_params, sparse_size))
                        print('Total parameters of dense model: {0}'.format(self.dense_params))
                        self.sparsity_level = 1 - sparse_size/ self.dense_params
    ...
    def pruning_uniform(self, step):
        ...
        if curr_prune_iter >= ini_iter and curr_prune_iter <= final_iter:
            ...
            if curr_prune_rate >= 0.8:
                print("self.fc_params = ", self.fc_params)
                print("self.dense_params = ", self.dense_params)
                print("self.total_params = ", self.total_params)
            ...
            print('Sparsity after pruning: {0}'.format(
                (total_size-sparse_size) / self.dense_params))
    ...
    def add_module(self, module, sparse_init='ERK', grad_dic=None):
        ...
        for name, tensor in module.named_parameters():
            ...
            if len(tensor.size()) >=2: # and tensor.grad != None:
    ...
    def truncate_weights(self, step=None):
        ...
        for module in self.modules:
            for name, weight in module.named_parameters():
                ...
                #print(name,self.pruning_rate[name])
                #print(weight.grad,weight.requires_grad)
    ...
    def gradient_growth(self, name, new_mask, total_regrowth, weight):
        if weight.grad == None:
            print("Random regrowth for ======>>> ", name)
            n = (new_mask==0).sum().item()
            if n == 0: return new_mask
            expeced_growth_probability = (total_regrowth/n)
            new_weights = torch.rand(new_mask.shape).cuda() < expeced_growth_probability #lsw
    ...
    def truncate_weights_grow(self, step=None):
        self.gather_statistics()
        # prune
        for module in self.modules:
            for name, weight in module.named_parameters():
        ...
        for module in self.modules:
            for name, weight in module.named_parameters():
                ...
                #    continue
                new_mask = self.masks[name].data.byte()
                new_mask = self.gradient_growth(name, new_mask, int(self.pruning_rate[name]*self.args.prune_rate_factor), weight)
                # exchanging masks
                self.masks.pop(name)
                self.masks[name] = new_mask.float()
        self.apply_mask()
    ...
    def truncate_weights_prune(self, step=None):
        self.gather_statistics()
        # prune
        for module in self.modules:
            for name, weight in module.named_parameters():
                ...
                #    continue
                mask = self.masks[name]
                new_mask = self.magnitude_death_prune(mask, weight, name)
                self.pruning_rate[name] = int(int(self.name2nonzeros[name] - new_mask.sum().item())*(1/self.args.prune_rate_factor))
                self.masks[name][:] = new_mask
        # grow
        for module in self.modules:
            for name, weight in module.named_parameters():
    ...
    def magnitude_death_prune(self, mask, weight, name):
        num_remove = math.ceil(self.prune_rate*self.name2nonzeros[name]*self.args.prune_rate_factor)
        if num_remove == 0.0: return weight.data != 0.0
        num_zeros = self.name2zeros[name]
        k = math.ceil(num_zeros + num_remove)
        x, idx = torch.sort(torch.abs(weight.data.view(-1)))
